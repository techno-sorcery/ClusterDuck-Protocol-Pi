...
void setup() {
    ...
    if (duck.setupWithDefaults(devId) != DUCK_ERR_NONE) {
        ...
        printf("[MAMA] Failed to setup MamaDuck");
        return;
    }
...
bool runSensor(void *) {
    ...
    if (result) {
        ...
        printf("[MAMA] runSensor ok.");
    } else {
        // Serial.println("[MAMA] runSensor failed.");
        printf("[MAMA] runSensor failed.");
    }
    return result;
}
 ...
 */ bool sendData(std::string message, byte topic) {
     ...
     if (!sentOk) {
         ...
         printf(("[Link] Failed to send data. error = %s", std::to_string(err)).c_str());
     }
     return sentOk;
 }
